    ...
    <Slider {...settings}>
      {products.map((product) => (
        <div key={product.id} className="card-mobile">
          ...
          <div className='new-bg-img-hss-mobile'>
            <h2><b>{product.title}</b></h2>
            <p className='new-description-mobile'>{product.description}</p>
            <Link to={product.url}><div className='new-bg-img-hss-url-mobile'>+</div></Link>
          </div>
        </div>
      ))}
    </Slider>
